...
#define INCH_TO_POINTS(in) (in * 72.27)
...
static struct OutputFormat formats[] =
  {
    ...
    {"Portable Document Format (PDF)", "pdf", OUTPUT_POINTS | OUTPUT_MULTIPAGE,
     export_pdf},
    {"Scalable Vector Graphics (SVG)", "svg", OUTPUT_POINTS | OUTPUT_MULTIPAGE,
     export_svg},
    {NULL, NULL, 0, NULL},
  };
...
export_page (PAGE *page)
{
  ...
  cairo_matrix_init (&render_mtx, xscale, 0, 0, - yscale,
                     - left * xscale,
                     yscale * top + height);
  cairo_set_matrix (eda_renderer_get_cairo_context (renderer), &render_mtx);
...
export_ps (void)
{
  ...
  /* Output setup section of Postscript file */
